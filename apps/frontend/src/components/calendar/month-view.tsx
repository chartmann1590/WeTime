'use client'
import { addDays, getMonthStart, getDaysInMonth, isSameDay, formatDate, filterEventsByOwner } from '@/lib/utils'
...
export function MonthView({ 
  ...
  const getEventsForDay = (date: Date, dayEvents: Event[]) => {
  ...
  const renderMonthColumn = (columnEvents: Event[], userName: string) => {
    return (
      <div className="flex-1 overflow-y-auto border-r border-border last:border-r-0">
        ...
        <div className="grid grid-cols-7">
          {Array.from({ length: weeks * 7 }, (_, i) => {
            ...
            const dayEvents = isCurrentMonth ? getEventsForDay(date, columnEvents) : []
  ...
  if (sideBySide && partnerId && partnerName) {
    ...
    return (
      <div className="flex-1 overflow-hidden flex">
        {renderMonthColumn(userEventsWithShared, currentUserName || 'You')}
        {renderMonthColumn(partnerEventsWithShared, partnerName)}
      </div>
    )
  }

  return renderMonthColumn(events, '')
}
