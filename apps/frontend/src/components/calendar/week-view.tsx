'use client'
import { addDays, formatTime, isSameDay, getStartOfDay, filterEventsByOwner } from '@/lib/utils'
...
export function WeekView({ 
  ...
  const getEventsForDay = (day: Date, dayEvents: Event[]) => {
  ...
  const renderWeekColumn = (columnEvents: Event[], userName: string) => {
    return (
      <div className="flex-1 overflow-y-auto border-r border-border last:border-r-0">
        ...
        <div className="relative min-h-[1200px]">
          ...
          {/* Events */}
          {weekDays.map((day, dayIndex) => {
            const dayEvents = getEventsForDay(day, columnEvents)
  ...
  if (sideBySide && partnerId && partnerName) {
    ...
    return (
      <div className="flex-1 overflow-hidden flex">
        {renderWeekColumn(userEventsWithShared, currentUserName || 'You')}
        {renderWeekColumn(partnerEventsWithShared, partnerName)}
      </div>
    )
  }

  return renderWeekColumn(events, '')
}
